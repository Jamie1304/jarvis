...
class CameraController:
    ...
    async def list_devices(self) -> tuple[CameraDevice, ...]:
        try:
            ...
            self._status = CameraStatus(CameraState.ERROR, None, str(error))
            raise
    ...
    async def capture_once(
        ...
        *,
        ...
        async with self._lock:
            ...
            try:
                self._status = CameraStatus(CameraState.OPENING, device_id, "Opening camera")
                session = await self.provider.open_device(device_id)
                self._session = session
                self._status = CameraStatus(CameraState.ACTIVE, device_id, "Camera capture active")
                return await self._capture_with_guards(session, timeout_seconds, cancellation)
            except asyncio.CancelledError as error:
                self._status = CameraStatus(CameraState.INACTIVE, device_id, "Capture cancelled")
                raise CameraCaptureCancelledError("Camera capture was cancelled") from error
            except TimeoutError as error:
                self._status = CameraStatus(
                    CameraState.ERROR, device_id, "Camera capture timed out"
                )
                raise CameraCaptureTimeoutError("Camera capture timed out") from error
            except CameraProviderError as error:
                self._status = CameraStatus(CameraState.ERROR, device_id, str(error))
                raise
            finally:
                self._session = None
                if session is not None:
                    try:
                        ...
                        if self._status.state is CameraState.ACTIVE:
                            self._status = CameraStatus(
                                CameraState.INACTIVE, device_id, "Camera is inactive"
                            )
                    ...
                    self._status = CameraStatus(
                        CameraState.ERROR, device_id, "Camera could not open"
                    )
        ...
        async with self._lock:
            ...
            if session is not None:
                try:
                    ...
                    self._status = CameraStatus(CameraState.INACTIVE, None, "Camera shut down")
