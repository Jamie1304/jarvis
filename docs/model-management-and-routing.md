...
`RoutingFitnessProjection` is a bounded view backed in production by the
separate `SQLiteRoutingFitnessStore`; isolated tests may use its explicitly
ephemeral in-memory mode. `PlanningEngine` records an operational result for each
...
optimization. The store owns only empirical route-observation history and is
not a second durable task ledger: `PlanningStore` owns task/plan truth, verification owns semantic proof,
