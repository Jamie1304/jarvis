...
| 42 | Runtime Behavior Drift | **PASS WITH RESTRICTIONS** | `RuntimeContainer` binds `CapabilityHealthService` activation reads/transitions to the canonical `PackageActivationService`/`SQLiteCapabilityLifecycleStore`. Certified baselines and package version/hash bindings are restored durably; stale observations fail closed; material/security drift durably degrades/quarantines and the registry/hot-load projection is not an authority. Trusted broker/application evidence is required; arbitrary hostile-process observation and universal third-party repair remain outside the v1 claim. |
...
| 58 | Authoritative-store map consistency | **PASS WITH RESTRICTIONS** | Capability/package lifecycle, certification, activation and behavior-baseline references have one durable `SQLiteCapabilityLifecycleStore`; health/drift is explicitly an ephemeral projection bound to that authority. Registry, hot-load and ComponentDoctor cannot write competing activation truth. Workspaces/profiles and hardware/model inventory remain explicitly non-durable v1 inputs where restart persistence is not required. |
