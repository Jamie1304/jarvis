...
| Workflow/procedure learning | `WorkflowTemplateRegistry`, `ProcedureBank`, `SQLiteWorkflowProcedureStore` | IMPLEMENTED_AND_WIRED | Templates instantiate plans through PlanValidator/PlanningEngine. Runtime-owned durable versions, sanitized trusted observations, candidate linkage, scoped context hints, user lifecycle, and restart reuse are covered by deterministic composition tests. |
