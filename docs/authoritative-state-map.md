...
| Routing empirical fitness and bounded resilience observations | `AUTHORITATIVE_DURABLE` | `RoutingFitnessProjection` and `RoutingResilienceService` over `SQLiteRoutingFitnessStore`, `routing-fitness.sqlite3`; schema v3; non-model route observations, route-scoped breaker state, and bounded exploration budget | `SQLiteRoutingFitnessStore` | Bounded observation, cooldown, and exploration policy; no raw prompts, payloads, secrets, or reasoning | No: retained observation history and resilience state are not fully reconstructible from planning snapshots | R3D-C quality views and R3D-D resilience selection | `PlanningStore`, `TraceStore`, ProviderRegistry health, model Cookbook, model/tool claims, UI |
