...
| Routing empirical fitness, bounded resilience, and decision evidence | `AUTHORITATIVE_DURABLE` | `RoutingFitnessProjection`, `RoutingResilienceService`, and immutable `RoutingDecisionRecord`/linked outcome records over `SQLiteRoutingFitnessStore`, `routing-fitness.sqlite3`; schema v4; non-model route observations, route-scoped breaker state, bounded exploration budget, safe selected-versus-executed routing facts | `SQLiteRoutingFitnessStore` | Bounded observation, cooldown, exploration, decision, and outcome retention; no raw prompts, outputs, secrets, or reasoning | No: retained observation, resilience, and decision history are not fully reconstructible from planning snapshots | R3D-C quality views, R3D-D resilience selection, R3D-F explainability/fusion | `PlanningStore`, `TraceStore`, PermissionBroker, VerificationEngine, model Cookbook, model/tool claims, UI |
...
| Live capability health and drift reports | `AUTHORITATIVE_EPHEMERAL` | `CapabilityHealthService` is a bounded health/drift projection. Its activation reads/transitions are bound by `RuntimeContainer` to `PackageActivationService`/`SQLiteCapabilityLifecycleStore`; its private activation map is compatibility-only for uncomposed unit tests. Certified behavior baselines are rehydrated from the lifecycle record and bind package version/hash. Only trusted application-owned broker/health/evidence producers may submit observations; package/model claims are rejected. | Lifecycle migration for persisted package health summary, certification and baseline reference | Current observations and bounded attention/trace history; re-probe after restart | Yes by re-probing; unknown remains unknown | ComponentDoctor, Attention, Control Center, lifecycle containment | model claim, package self-report, UI, in-memory registry |
| Repair cases and attempt outcomes | `AUTHORITATIVE_DURABLE` | `SQLiteRepairStore`, `repair.sqlite3`, schema v1; `ComponentDoctor` is the sole repair state machine | `SQLiteRepairStore` plus trusted application callback bindings re-established at startup | Bounded case and attempt retention; no prompts, credentials, callbacks, or raw payloads | In-flight effects reconcile to `QUARANTINED`; terminal cases remain historical | ComponentDoctor, health projection, Trace, Attention, Control Center | health status, model prose, executor self-report, UI |
...
| Resource admission/reservations | `AUTHORITATIVE_EPHEMERAL` | One `ResourceGovernor` in `RuntimeContainer`; reservations exist only while work runs; interactive RAM/VRAM/CPU/concurrency headroom is protected and candidate budgets use trusted local metadata/measurements | None | Release on complete/cancel/crash/timeout | Yes from current telemetry; reservations are not restart truth | ProviderRouter, InferenceDispatcher, CapabilityFactory, Sandbox, warmup, multi-agent | model/provider, scheduler, UI |
...
Capability activation and drift use one write path: `CapabilityHealthService`
classifies trusted observations, while the composed lifecycle callbacks invoke
the durable activation service. A drift result never writes an independent
activation authority. Behavior evidence must identify the certified package
version and hash; stale-version observations fail closed. ComponentDoctor may
diagnose and propose/execute an owner-approved repair, but it cannot clear a
quarantine or promote a package independently of the lifecycle store.
