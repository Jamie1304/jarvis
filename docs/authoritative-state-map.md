...
| Integration package boundaries and static review | Validated `IntegrationPackage` contract plus native `GeneratedPackageReviewer`; package lifecycle remains application/trusted-policy owned; generated execution uses transient `SandboxProcess` instances | Package code manifests/source hashes, external user config/data, Vault references, rebuildable cache, diagnostics, migrations, provenance, review findings, sandbox IPC observations | IMPLEMENTED as contracts and review/lifecycle primitives. Package code is immutable/hash-bound; unreviewed source is manual-review-only; unsafe hooks, execution, deserialization, process/network bypasses, traversal, secret logging, and approval spoofing are rejected. User config and package data are external and preserved across update/uninstall; credentials are references only; the reviewer never executes code or changes reviewer/policy/approval authority. `SandboxProcess` provides bounded typed IPC and Windows Job Object process/resource ownership, but is not a complete filesystem/network/AppContainer authority. |
