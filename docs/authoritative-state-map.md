...
| Hardware and model inventory | `HardwareInventoryService` for timestamped probe snapshots and `ModelInventory` for descriptive model metadata/measurements; no durable store yet | `HardwareProfile`, provider definitions, compatibility evaluations, model-combination plans, and CI fake readings | IMPLEMENTED as process-local empirical metadata. `None` means unmeasured; `PUBLISHED`, `COMMUNITY`, and `MEASURED_ON_THIS_MACHINE` evidence remain distinct. `ModelPlanner` cannot download, load, activate, authorize, or claim portability; unknown capacity is never treated as compatibility. A future durable inventory store must be added here before persistence is introduced. |
