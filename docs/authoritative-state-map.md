...
| Integration package boundaries, review, and certification | Validated `IntegrationPackage`, native `GeneratedPackageReviewer`, and `PackageCertifier`; package lifecycle remains application/trusted-policy owned; generated execution uses transient `SandboxProcess` instances | Package code manifests/source hashes, external user config/data, Vault references, rebuildable cache, diagnostics, migrations, provenance, review findings, certification records, sandbox IPC observations | IMPLEMENTED as contracts and review/certification/lifecycle primitives. Package code is immutable/hash-bound; unreviewed source is manual-review-only; unsafe hooks, execution, deserialization, process/network bypasses, traversal, secret logging, and approval spoofing are rejected. `CertificationRecord` binds exact source/dependency/manifest/package hashes, permissions, test/audit/health/verification evidence, authority approval, rollback target, and Shadow/Canary eligibility. CERTIFIED is distinct from ACTIVE; user config and package data remain external and preserved across update/uninstall; credentials are references only. |
