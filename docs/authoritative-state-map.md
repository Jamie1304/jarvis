...
| Capability/integration metadata | Sealed runtime `ToolRegistry`, trusted catalogs, and pre-seal `MCPExtensionManager` configuration/lifecycle | Capability discovery candidates, generated knowledge, MCP descriptions/results, events | IMPLEMENTED for native MCP consumption; runtime state remains in-memory and configuration is trusted local input. MCP servers are untrusted, namespaced adapters only, cannot collide, register after sealing, grant permissions, or become a second authority. |
