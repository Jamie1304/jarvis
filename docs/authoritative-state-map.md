...
| Artifacts | `ScreenshotStore`, `EphemeralFrameStore`, and `TestArtifactStore` for their respective bounded lifetimes | Opaque references and metadata | PARTIAL. Current screenshot/camera stores are ephemeral and test artifacts are controlled evidence; no general durable artifact authority exists. Do not introduce one without updating this map. |
| Capability/integration metadata | Sealed runtime `ToolRegistry` and trusted catalogs | Capability discovery candidates, generated knowledge, events | PARTIAL/in-memory. Registry state is runtime composition, not a durable user-created integration database. Discovery candidates are untrusted evidence and cannot register or authorize tools. |
