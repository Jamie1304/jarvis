...
| R2A-M09 certification/drift/repair provenance | `MITIGATED` | The composed runtime now binds drift activation reads/transitions to the durable lifecycle service; certified baselines include the package hash and observations must match version/hash. ComponentDoctor cannot promote or clear lifecycle state. Cross-process authenticated provenance and broader independent observation remain follow-up outside the generic in-process v1 path. |
