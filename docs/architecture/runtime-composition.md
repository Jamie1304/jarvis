...
| `SemanticEventService` / `SemanticPatternEngine` | `PRODUCTION_OWNED` | Bounded process-local projection at `RuntimeContainer.semantic_events` over the canonical `EventBus` | Deterministic descriptive observations only; no model classification, persistence, memory, Attention, or authority |
| `EpisodeComposer` / `EpisodeStore` | `PRODUCTION_OWNED` | `RuntimeContainer.episode_composer` over the existing `SQLiteMemoryStore` and canonical task services | One deterministic, actor/workspace-scoped Episode at an authoritative terminal boundary; compact evidence references only, no raw history, model inference, Attention, or authority |
